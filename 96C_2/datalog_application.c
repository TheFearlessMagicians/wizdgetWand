...
void Accelero_Sensor_Handler( void *handle, uint32_t msTick, uint32_t *msTickStateChange, uint8_t *state )
{
  ...
  uint32_t tau =1500;
  float z_up_thresh = 1400.0f;
  float z_down_thresh = 100.0f;
  float xy_idle_thresh = 900.0f;

  float x_right_thresh = 800.0f;
  float yz_idle_thresh = 1500.0f;
  ...
  if ( status == 1 )
  {
    ...
    {
//    	uint32_t abs_acc;
//    	      abs_acc = ((int)acceleration.AXIS_X * (int)acceleration.AXIS_Y);
...
//      CDC_Fill_Buffer((uint8_t *)dataOut, strlen(dataOut));
      ...
      if((*state) == 0 && (z > z_up_thresh) && fabs(x) < xy_idle_thresh && fabs(y) < xy_idle_thresh){
    	  ...
    	  *state = 6;
    	  ...
    	  sprintf( dataOut,"\n-\n-\n \n EXPELLIARMUS DETECTED EXPELLIARMUS \n-\n-\n-\n"
    	      	     	        		      		       );
    	      	     	  CDC_Fill_Buffer((uint8_t *)dataOut, strlen(dataOut));
